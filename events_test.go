...
func seed(t *testing.T, repo *eventmongorepo.Repo, playerID string, base time.Time, types ...string) {
	t.Helper()
...
func TestListRecent_OrderLimitAndTypeFilter(t *testing.T) {
	repo := eventmongorepo.NewRepo(testMongo.Database("profiles"))
	...
	t.Cleanup(func() {
		testMongo.Database("profiles").Collection("events").Drop(context.Background())
	})
}
